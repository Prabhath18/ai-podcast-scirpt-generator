...
- **Generating Outline.** While an outline is generated the New Episode screen shows a card with the title, an estimated progress bar, four steps (Analysing podcast strategy, Structuring narrative flow, Drafting guest questions, Synthesising outline) and a skeleton of the outline to come. For a single outline the card follows the streamed output: the bar is the server's count of finished parts of the outline (title, intro, each segment, guest questions, outro), the current step follows the stage and never moves backwards (it starts over only when the server says it is retrying), and a plain sentence under the bar says what was just written ("3 of about 6 segments drafted."). Before the first token it reads "Waiting for the model to start writing." at 0%, which is honest but can last several seconds. Its `aria-label` is "Generation progress". When streaming is unavailable (two or three structures, an older server or an unsupported browser, or a stream that broke) the card uses the earlier timer-based estimate: the steps advance on a timer and the bar levels off below 100% until the response arrives, and its `aria-label` says "(estimated)". Screen readers get "Step 2 of 4: Structuring narrative flow. 3 of about 6 segments drafted." in live mode and "Step 2 of 4: Structuring narrative flow" in estimate mode.
